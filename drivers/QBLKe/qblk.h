...
/* Uncomment to trace ring buffer change */
//#define QBLK_TRACE_RB_CHANGE

/* Uncomment to count user requests issued to rb */
//#define QBLK_COUNT_RB_REQ
...
#define QBLK_RB_MAX_COMPLAINT (2048)

#define QBLK_COMPLAINT_THRESHOLD (32)
#define QBLK_RB_MAX_FREE_PER_TURN (48)
...
struct qblk_rb_entry {
	...
	struct qblk_rb_pages *page_set; /* Which page_set does this entry belong to */
	unsigned int offset;		/* offset inside page_set */
...
struct qblk_rb {
	...
	/* Updating boundary & boundary_small are protected by w_lock*/
	unsigned int boundary;
	unsigned int boundary_large;
	unsigned int boundary_small;

	atomic_t complaint;
	int rb_active;
	int rb_active_cache;
	struct delayed_work shrink_dwork;
...
struct qblk {
	...
	unsigned int free_pool_threshold;
	struct qblk_rb *mqrwb;
	unsigned int rb_min_thres;
	unsigned int rb_mid_thres;
	unsigned int rb_max_thres;

	struct delayed_work rb_coordinator;
	atomic_t run_coordinator;
	unsigned int *complaints;
	int *op_saved;
	int op_saved_iter;
	...
	int gc_active_size;
	unsigned long *gc_active; //TODO: use atomic bitmask
...
extern unsigned int qblk_rb_read_commit(struct qblk_rb *rb,
				unsigned int nr_entries);
extern inline struct qblk_w_ctx *qblk_rb_w_ctx(struct qblk_rb *rb, unsigned int pos);
...
extern int qblk_init_global_rb(struct qblk *qblk, unsigned int order, unsigned int iter);
extern int qblk_alloc_freepage_to_rb(struct qblk *qblk,
				struct qblk_rb *rb, unsigned int startEntry, unsigned int nr_entries);
extern void qblk_free_page_from_rb(struct qblk *qblk,
					struct qblk_rb *rb,
					unsigned int startEntry,
					unsigned int endEntry);
extern void qblk_free_global_rb(struct qblk *qblk);
extern int qblk_rb_init(struct qblk *qblk, struct qblk_rb *rb,
...
extern void qblk_rb_coordinator_fn(struct work_struct *work);
/*
 * qblk gc
 */
...
/* qblk-debug.c */
extern void qblk_printBioStatus (struct bio *bio);
...
extern void qblk_debug_time(struct qblk *qblk, int *pindex,
				int chnl, struct qblk_debug_entry entry);
extern void qblk_debug_printBioStatus(struct bio *bio);
extern void qblk_debug_init(struct qblk *qblk);
extern void qblk_debug_exit(void);
#ifdef QBLK_TRACE_RB_CHANGE
extern void qblk_trace_rbChange(int rb_index, int isshrink, int changeValue);
#endif
