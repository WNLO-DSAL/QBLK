...
/* usage: "be @rb_index @nr_pages"*/
static void qblk_debugBuffer_extend(struct qblk *qblk, char *usrCommand)
{
	int rb_idx;
	int nr_pages;
	struct qblk_rb *rb;
	...
	sscanf(usrCommand, "%d%d", &rb_idx, &nr_pages);

	rb = &qblk->mqrwb[rb_idx];
	spin_lock(&rb->w_lock);
	boundary = READ_ONCE(rb->boundary);
	qblk_alloc_freepage_to_rb(qblk, rb, boundary, nr_pages);
	smp_store_release(&rb->boundary_large, boundary + nr_pages);
	//smp_store_release(&rb->boundary, boundary + nr_pages);
	spin_unlock(&rb->w_lock);
}

/* usage: "bs @rb_index @nr_pages"*/
static void qblk_debugBuffer_shrink(struct qblk *qblk, char *usrCommand)
{
	int rb_idx;
	int nr_pages;
	struct qblk_rb *rb;
	...
	spin_lock(&rb->w_lock);
	boundary = READ_ONCE(rb->boundary);
	smp_store_release(&rb->boundary_small, boundary - nr_pages);
	spin_unlock(&rb->w_lock);
}
...
static void qblk_debugBuffer_redundancy(struct qblk *qblk, char *usrCommand)
{
	...
	rb = &qblk->mqrwb[rb_idx];
	entry = &rb->entries[entry_index];
	pr_notice("%s, dataP=%p;pagesetP=%p;offset=%u\n",
			__func__, entry->data,
			entry->page_set, entry->offset);
				...
				size_t count, loff_t *ppos)
{
	...
	case 'b':
		pr_notice("%s, b\n", __func__);
		if (usrCommand[1] == 'e')
			qblk_debugBuffer_extend(qblk, &usrCommand[2]);
		else if (usrCommand[1] == 's')
			qblk_debugBuffer_shrink(qblk, &usrCommand[2]);
