...
static int qblk_rwb_mq_init(struct qblk *qblk, unsigned int queue_count)
{
	...
	unsigned long init_entries;
	unsigned int i;
	int ret;
	struct qblk_rb *rwbArray;
	unsigned int max_order = MAX_ORDER - 1;
	unsigned int order;
	unsigned int alloc_order, iter;
	int luns_per_chnl = qblk->dev->geo.nr_luns;

	if(qblk->pgs_in_buffer < ((luns_per_chnl+1) * qblk->min_write_pgs + 1) * queue_count)
		qblk->pgs_in_buffer = ((luns_per_chnl+1) * qblk->min_write_pgs + 1) * queue_count;

	//qblk->rb_min_thres = 2;
	qblk->rb_min_thres = qblk->min_write_pgs + QBLK_MIN_THRES_BONUS;
	qblk->rb_mid_thres = qblk->pgs_in_buffer / queue_count;
	qblk->rb_max_thres = qblk->pgs_in_buffer;

	pr_notice("%s, rb_min/mid/max=%u,%u,%u\n",
					__func__, qblk->rb_min_thres,
					qblk->rb_mid_thres, qblk->rb_max_thres);

	alloc_order = get_count_order(qblk->pgs_in_buffer);
	...
	if(qblk_init_global_rb(qblk, order, iter))
		return -ENOMEM;

	rwbArray = qblk->mqrwb = kmalloc_array(queue_count, sizeof(struct qblk_rb), GFP_KERNEL);
	if (!rwbArray)
		goto err_out_free_globalrb;

	init_entries = (qblk->pgs_in_buffer - queue_count * qblk->min_write_pgs * luns_per_chnl)/queue_count;
	for (i = 0; i < queue_count; i++) {
		entries = vzalloc(qblk->rb_max_thres * sizeof(struct qblk_rb_entry));
		if (!entries)
			goto err_out_free_mem;

		ret = qblk_rb_init(qblk, &qblk->mqrwb[i], i,
					entries, init_entries, geo->sec_size);
#if 0
		if(!i)
			ret = qblk_rb_init(qblk, &qblk->mqrwb[i], i,
					entries, qblk->pgs_in_buffer - (queue_count-1)*(qblk->min_write_pgs+1), geo->sec_size);
		else
			ret = qblk_rb_init(qblk, &qblk->mqrwb[i], i,
					entries, qblk->min_write_pgs+1, geo->sec_size);
#endif
		if (ret) {
			vfree(entries);
			goto err_out_free_mem;
	...
	qblk->free_pool_threshold = atomic_read(&qblk->nr_free_pages);
	pr_notice("%s, free_pool_threshold=%u\n", __func__, qblk->free_pool_threshold);
	qblk->complaints = kmalloc_array(qblk->nr_queues, sizeof(unsigned int), GFP_KERNEL);
	if (!qblk->complaints)
		goto err_out_free_mem;
	qblk->op_saved = kmalloc_array(qblk->nr_queues * QBLK_NR_SAVED_OP, sizeof(int), GFP_KERNEL);
	if (!qblk->op_saved)
		goto err_out_free_mem2;
	memset(qblk->op_saved, 0, sizeof(int) * qblk->nr_queues * QBLK_NR_SAVED_OP);
	qblk->op_saved_iter = 0;

	INIT_DELAYED_WORK(&qblk->rb_coordinator, qblk_rb_coordinator_fn);
	atomic_set(&qblk->run_coordinator, 1);
	...
	kfree(qblk->op_saved);
err_out_free_mem2:
	kfree(qblk->complaints);
err_out_free_mem:
	qblk_rwb_mq_free(qblk, i);
err_out_free_globalrb:
	qblk_free_global_rb(qblk);
	return -ENOMEM;
}
							...
							struct nvm_ioctl_create *create)
{
	...
	/* These two values must be calculated here so that qblk_rwb_mq_init can use them*/
...
//	blk_queue_flag_set(QUEUE_FLAG_NOMERGES, blk_queue);
	 ...
	 * but user and GC I/O
	 */
	ret = qblk_rl_init(&qblk->rl, qblk->pgs_in_buffer);
	if (ret) {
...
static void qblk_free(struct qblk *qblk)
{
	...
	blk_mq_free_tag_set(qblk->tag_set);
	qblk_cleanup_queues(qblk);
	atomic_set(&qblk->run_coordinator, 0);
	while (atomic_read(&qblk->run_coordinator) != 2) schedule();
	kfree(qblk->op_saved);
	kfree(qblk->complaints);
